...
class ScheduledUpdateTypeForm extends ScheduledUpdateTypeBaseForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        if ($multi_field) {
          $this->messenger()->addStatus($this->t('Select fields to add to these updates'));
          $form_state->setRedirectUrl($this->entity->toUrl('clone-fields'));
        }
        else {
      ...
      default:
        $this->messenger()->addStatus($this->t('Saved the %label Scheduled update type.', [
