...
abstract class ScheduledUpdatesTestBase extends WebTestExtended {
  ...
  protected function cloneFields($type_id, array $fields) {
    ...
    foreach ($fields as $input_name => $field_info) {
      ...
      $this->assertSession()->pageTextContains($field_info['label']);
      // Add to post data.
      $edit[$input_name] = $field_info['input_value'];
    }
    $this->submitForm($edit, t('Clone Fields'));
    if ($this->adminUser->hasPermission('administer scheduled_update form display')) {
      ...
      $this->assertSession()->addressEquals("admin/config/workflow/scheduled-update-type/$type_id/form-display");
      ...
      $this->assertSession()->pageTextContains('You do not have permission to administer fields on Scheduled Updates.');
    }
  ...
  protected function createType($label, $id, array $clone_fields, $type_options = []) {
    ...
    $this->assertSession()->pageTextNotContains('The owner of the last revision.');
    $this->assertSession()->pageTextNotContains('Create New Revisions');
    $edit = $type_options + [
        'label' => $label,
        'id' => $id,
    ...
    $this->assertSession()->pageTextContains('The owner of the last revision.');
    $this->assertSession()->pageTextContains('Create New Revisions');
    $edit = $type_options + [
        'label' => $label,
        'id' => $id,
    ...
    $this->assertSession()->addressEquals("admin/config/workflow/scheduled-update-type/$id/clone-fields");
    $this->assertSession()->pageTextContains("Created the $label Scheduled Update Type.");
    $this->assertSession()->pageTextContains("Select fields to add to these updates");
  ...
  protected function grantPermissionsToUser($permissions) {
    ...
    $this->assertTrue('debug', "roles =" . implode(',', $roles));
